...
Future<void> showOnExternalMap(BuildContext context, double latitude, double longitude, String title) async {
...
Future<AvailableMap?> showAvailableMapsDialog(BuildContext context, List<AvailableMap> availableMaps) async {
  return await showListItemPickerBottomSheet<AvailableMap>(
    ...
    builder: (map) => Padding(
      ...
      child: Row(spacing: 4, children: [SvgPicture.asset(map.icon, height: 30.0, width: 30.0), AppText(map.mapName)]),
    ),
  );
}

Future<void> leaveReview(BuildContext context, Booking booking) async {
  context.read<OldBookingsCubit>().startReviewFlow(context, booking);
}
