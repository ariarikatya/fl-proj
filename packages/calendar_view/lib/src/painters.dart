...
class CurrentTimeLinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (showBullet) {
      canvas.drawRect(Rect.fromLTWH(offset.dx, offset.dy - 7, 1.5, 14), Paint()..color = color);
      // canvas.drawCircle(
    ...
    if (showTimeBackgroundView) {
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(max(3, offset.dx - 68), offset.dy - 11, timeBackgroundViewWidth, 24),
          Radius.circular(12),
        ),
        Paint()
          ..color = color
    ...
    if (showTime) {
      TextPainter(
          ...
          text: TextSpan(
            text: timeString,
            style: TextStyle(fontSize: 12.0, color: showTimeBackgroundView ? Colors.white : color),
          ),
        )
        ..layout()
